  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.questionText}>What emotion have you noticed?</Text>
...
<FlatList
  ...
  contentContainerStyle={{
    paddingHorizontal: (width - ITEM_TOTAL_WIDTH) / 2, // center padding
  }}
  ...
  onMomentumScrollEnd={(e) => {
    ...
    const index = Math.round(offset / ITEM_TOTAL_WIDTH);
    setSelectedEmotionId(emotions[index]?.id);
  }}
  renderItem={({ item, index }) => {
    const isCentered = item.id === selectedEmotionId;
    return (
      <TouchableOpacity
        style={{
          width: ITEM_WIDTH,
          ...
          paddingHorizontal: ITEM_SPACING, // add spacing *inside* item
        }}
        onPress={() => {
...
/>
        ...
        <View>
          <Text style={styles.sectionTitle}>Discomfort intensity</Text>
          <Slider
            style={styles.slider}
            minimumValue={0}
            maximumValue={1}
            value={discomfortIntensity}
            onValueChange={setDiscomfortIntensity}
